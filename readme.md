...
Insert the Fingerprint Sensor into the front plate from the front by sliding it in, and apply hot glue gun to fix it there, and using soldering iron, insert the heatset insert into the front plate and then place the led on a circular cardboard and place them there with superglue aand wire them up and then place them from the behind matching with holes provided in the circular ring. now keep the front and back open of the enclosure and place the esp32 and the relay and buck converter, hot glue them to fix them in place, and do the wiring as mentioned in the wiring diagram, and then close the frontplate and screw it in now run the 12v power wires, exit switch wire, solenoid door lock wires, from the hole at the back and wire them up with their respective part. for diffusion, i will place a tanslucent acrylic or plastic sheet by cutting it in circular shape and insert it in, and fill the groove with hot glue gun using the white glue stick which is nearly opaque. And then connect the esp32 with pc to flash the code, and then close up the back panel, it will just fir in right there by pushing it in.
